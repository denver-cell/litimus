...
export default function Detector() {
  ...
  const [dailyBlocked, setDailyBlocked] = useState<{
    reason: string;
    wordsUsedToday?: number;
    dailyLimit?: number;
  } | null>(null);
  // Today's free-quota snapshot, shown as a standing "X of 2,000 words
  // left today" line so visitors know the allowance up front rather than
  // discovering it only once they're blocked. Fetched read-only on mount
  // (see the effect below) and refreshed after every scan attempt.
  const [todayUsage, setTodayUsage] = useState<AnonUsageSnapshot | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const backdropRef = useRef<HTMLDivElement>(null);

  const wordCount = useMemo(() => words(text).length, [text]);
  // Instant, no-network hint: a single passage over the free plan's whole
  // daily allowance is guaranteed to fail the real check below even on a
  // fresh day, so this catches the obvious case immediately without a
  // round trip. It is NOT the actual enforcement — see dailyBlocked.
  const overFreeLimit = wordCount > FREE_WORD_LIMIT;
  // The real enforcement: on every scan attempt, /api/usage/check-anon
  // reports just the word count (never the manuscript text) to the
  // backend, which checks and records it against a 2,000-word/24h total
  // keyed by the visitor's hashed IP (backend/lib/usageLimiter.ts) — the
  // same bucket used for anonymous /api/detect callers. That makes the
  // cap a real cross-tab, cross-session daily total instead of something
  // reset by opening a new tab, while the actual scoring still happens
  // locally in the browser (see runScan below).
  const blocked = overFreeLimit || !!dailyBlocked;
  const verdict = result ? verdictLabel(result.score) : null;
  const tips = result ? improvementTips(result) : [];

  useEffect(() => {
    // Any edit invalidates the last scan's highlights and any stale
    // daily-limit block (the new word count needs its own fresh check).
    setBackdropHtml(escapeHtml(text) + "\n");
    setDailyBlocked(null);
  }, [text]);

  useEffect(() => {
    // Best-effort, read-only: show today's real remaining quota as soon
    // as the widget loads. If the backend is unreachable this just stays
    // null and the quota line falls back to the static "2,000/day" copy
    // below — never blocks the page or the client-side scoring.
    let cancelled = false;
    peekAnonUsage().then((snapshot) => {
      if (!cancelled && snapshot) setTodayUsage(snapshot);
    });
    return () => {
      cancelled = true;
    };
  ...
  async function runScan() {
    ...
    if (count > FREE_WORD_LIMIT) {
      // Belt-and-suspenders: the button is already relabeled past the
      // limit, but never run a scan on a passage over the free tier's cap.
      setResult(null);
      return;
    }

    // Ask the backend whether this scan fits in today's remaining free
    // quota BEFORE running it — this is the real, cross-session daily
    // cap (see the comment on `blocked` above). Only the word count is
    // sent; the passage itself is never transmitted at this step.
    setDailyBlocked(null);
    setCheckingQuota(true);
    let usage: AnonUsageCheck;
    try {
      usage = await checkAnonUsage(count);
    } catch {
      usage = {
        allowed: false,
        error: "Couldn't verify today's free limit — check your connection and try again.",
      };
    }
    setCheckingQuota(false);

    if (usage.wordsUsedToday !== undefined && usage.dailyLimit !== undefined) {
      // Keep the standing "X of 2,000 left today" line in sync with what
      // the backend just reported, whether this attempt was allowed or not.
      setTodayUsage({ wordsUsedToday: usage.wordsUsedToday, dailyLimit: usage.dailyLimit });
    }

    if (!usage.allowed) {
      setResult(null);
      setDailyBlocked({
        reason: usage.error || "You've reached today's free limit.",
  ...
  return (
    <div className="panel" id="detector">
      ...
      <div className="quota-line">
        {todayUsage ? (
          <>
            {Math.max(todayUsage.dailyLimit - todayUsage.wordsUsedToday, 0).toLocaleString()} of{" "}
            {todayUsage.dailyLimit.toLocaleString()} free words left today.
          </>
        ) : (
          <>{FREE_WORD_LIMIT.toLocaleString()} free words per day, no account needed.</>
        )}{" "}
        <Link href="/pricing">Need more?</Link>
      </div>
      <div className="scan-line" style={{ width: `${scanWidth}%` }} />
      <div className="panel-foot">
        <div className="foot-left">
          <span className="wordcount" style={blocked ? { color: "var(--ai)" } : undefined}>
            {wordCount} words{overFreeLimit ? ` (free limit: ${FREE_WORD_LIMIT.toLocaleString()})` : ""}
          </span>
        ...
        {blocked ? (
          <Link href="/pricing" className="btn">
            See plans for longer scans
          </Link>
        ) : (
          <button className="btn" onClick={runScan} disabled={scanning || checkingQuota}>
            {checkingQuota ? "Checking today's limit…" : scanning ? "Scanning…" : "Scan passage"}
          </button>
        )}
      </div>
      <div className="upload-status">{uploadStatus}</div>
      {blocked && (
        <div className="form-error" style={{ margin: "0 16px 16px" }}>
          {overFreeLimit ? (
            <>
              This passage is {wordCount.toLocaleString()} words — over the free plan&apos;s{" "}
              {FREE_WORD_LIMIT.toLocaleString()}-word limit. Upgrade to Student, Pro, or a one-time day pass to scan
              longer passages.
            </>
          ) : (
            <>
              {dailyBlocked?.wordsUsedToday !== undefined && dailyBlocked?.dailyLimit !== undefined ? (
                <>
                  You&apos;ve used {dailyBlocked.wordsUsedToday.toLocaleString()} of your{" "}
                  {dailyBlocked.dailyLimit.toLocaleString()} free words today.{" "}
                </>
              ) : null}
              {dailyBlocked?.reason} Your free quota resets daily at midnight UTC — or upgrade to Student, Pro, or a
              one-time day pass for more right now.
            </>
          )}
        </div>
      )}
